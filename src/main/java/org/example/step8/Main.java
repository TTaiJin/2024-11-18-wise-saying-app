...
public class Main {
    ...
    private static void register() {
        ...
        WiseSaying.getWsList().add(wsObj);
    ...
    private static boolean delete(int number) {
        for (int i = 0; i < WiseSaying.getWsList().size(); i++) {
            if (WiseSaying.getWsList().get(i).getIndex() == number) {
    ...
    private static boolean edit(int number) {
        for (int i = 0; i < WiseSaying.getWsList().size(); i++) {
            if (WiseSaying.getWsList().get(i).getIndex() == number) {
                System.out.println("명언(기존) : " + WiseSaying.getWsList().get(i).getWiseSaying());
                System.out.print("명언 : ");
                String editedWiseSaying = scanner.nextLine();
                WiseSaying.getWsList().get(i).setWiseSaying(editedWiseSaying);

                System.out.println("작가(기존) : " + WiseSaying.getWsList().get(i).getWriter());
                System.out.print("작가 : ");
                String editedWriter = scanner.nextLine();
                WiseSaying.getWsList().get(i).setWriter(editedWriter);
                return true;
            }
        }
        return false;
